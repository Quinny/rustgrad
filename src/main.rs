...
fn main() {
    let net = NeuralNet::new(vec![2, 3, 7, 1]);
    for _i in 0..1000 {
        let inputs = vec![
        ...
        let square_error = inputs
            ...
            .map(|(predicted, actual)| predicted[0].subtract(&actual).squared())
            .reduce(|x, y| x.add(&y))
            .unwrap();
        ...
        net.parameters().iter().for_each(|v| v.learn(0.0001));
    }

    net.dump();
    println!(
        "9 + 4 = {}",
        net.forward(vec![value(9.0), value(4.0)])[0].data()
    );
}
